...
public class OrganizeImports {
	...
	public String getOrganizedImportSection(XtextResource state) {
		...
		final SpecFile specFile = getSpecFile(state);
		if (specFile == null)
			return null;
		acceptor.addImplicitlyImportedPackages(newArrayList("java.lang", specFile.getPackageName()));
	...
	public TextRegion computeRegion(XtextResource resource) {
		final SpecFile specFile = getSpecFile(resource);
		if (specFile == null)
			return null;
		List<INode> packDecl = NodeModelUtils
				.findNodesForFeature(specFile, SpecPackage.Literals.SPEC_FILE__PACKAGE_NAME);
		int offset = 0;
		if (packDecl.size() >= 1)
			offset = packDecl.get(0).getOffset() + packDecl.get(0).getLength();
		if (specFile.getElements().isEmpty())
			return null;
		ICompositeNode node = NodeModelUtils.getNode(specFile.getElements().get(0));
		int length = node.getTotalOffset() - offset;
		return new TextRegion(offset, length);
	}

	protected SpecFile getSpecFile(XtextResource resource) {
		if (resource.getContents().isEmpty())
			return null;
		final EObject eObject = resource.getContents().get(0);
		if (!(eObject instanceof SpecFile))
			return null;
		final SpecFile specFile = (SpecFile) eObject;
		return specFile;
	}
	...
	public void collectAllReferences(XtextResource resource, ReferenceAcceptor acceptor) {
		SpecFile specFile = getSpecFile(resource);
		TreeIterator<EObject> contents = EcoreUtil.getAllContents(specFile, true);
