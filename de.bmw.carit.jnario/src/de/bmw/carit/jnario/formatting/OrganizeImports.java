 ...
 *******************************************************************************/
package de.bmw.carit.jnario.formatting;
...
public class OrganizeImports {
	...
	public TextRegion computeRegion(XtextResource resource) {
		final Jnario Jnario = getJnario(resource);
		if (Jnario == null)
			return null;
	...
	protected Jnario getJnario(XtextResource resource) {
		if (resource.getContents().isEmpty())
			return null;
		final EObject eObject = resource.getContents().get(0);
		if (!(eObject instanceof Jnario))
			return null;
		final Jnario Jnario = (Jnario) eObject;
		return Jnario;
	}
	...
	public void collectAllReferences(XtextResource resource, ReferenceAcceptor acceptor) {
		Jnario Jnario = getJnario(resource);
		TreeIterator<EObject> contents = EcoreUtil.getAllContents(Jnario, true);
